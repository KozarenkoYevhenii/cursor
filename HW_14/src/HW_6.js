...
const getBestStudent = (students) => {
  ...
  for (let student of students) {
    const studMark = getAverageMark(student);
    if (+studMark > maxMark) {
      maxMark = studMark;
...
//-------------------------Вивід результатів--------------------
function hw6() {
  document.body.innerHTML += "<strong>HW#6</strong> <br>";
  document.body.innerHTML += `1 Перелік предметів студента: ${getSubjects(
    students[0]
  )} <br>`;
  document.body.innerHTML += `2 Середня оцінка студента: ${getAverageMark(
    students[0]
  )} <br>`;
  document.body.innerHTML += `3 Загальна інформація про студента: виведено в консолі <br>`;
  console.log(getStudentInfo(students[0]));
  document.body.innerHTML += `4 Імена студентів у алфафітному порядку: ${getStudentsNames(
    students
  )} <br>`;
  document.body.innerHTML += `5 Ім'я кращого студента: ${getBestStudent(
    students
  )} <br>`;
  document.body.innerHTML += `6 Кількість букв у слові "test": виведено в консолі <br>`;
